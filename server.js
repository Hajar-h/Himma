...
const validateBrochure = [
  body("firstName").trim().escape().isLength({ min: 2, max: 25 }).withMessage("First name must be 2–25 letters."),
  body("lastName").trim().escape().isLength({ min: 2, max: 25 }).withMessage("Last name must be 2–25 letters."),
  body("workEmail").trim().normalizeEmail().isEmail().withMessage("Enter a valid email address."),
  body("phoneNumber").optional().trim().isLength({ min: 7, max: 15 }).isNumeric().withMessage("Phone must be 7–15 digits."),
  body("companyName").trim().escape().isAlphanumeric().isLength({ min: 2, max: 50 }).withMessage("Company name must be 2–50 characters."),
...
const validateContactUs = [
  ...
  body("company").optional({ checkFalsy: true }).trim().escape().matches(/^[A-Za-z0-9 ]{2,50}$/).withMessage("Company name must be 2–50 letters, numbers, or spaces."),
  body("phone").optional().trim().isLength({ min: 7, max: 15 }).isNumeric().withMessage("Phone must be 7–15 digits."),
...
app.post("/submit-form", validateContactUs, (req, res) => {
  ...
  const { ask, subject, message, title, firstName, lastName, dob, gender, language, country, company, email } = req.body;
  const phone = req.body.phone || null;

  const sql = 'INSERT INTO contact_us (ask, subject, message, title, firstName, lastName, dob, gender, language, country, company, phone, email) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)';
  const params = [ask, subject, message, title, firstName, lastName, dob, gender, language, country, company, phone, email];
