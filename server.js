...
// Brochure Form Validation and Handler
...
// Contact Us Form Validation and Handler
const validateContactUs = [
  ...
  body("company").optional({ checkFalsy: true }).trim().escape().matches(/^[A-Za-z0-9 ]{2,50}$/).withMessage("Company name must be 2–50 letters, numbers, and spaces only."),
...
app.post("/submit-form", validateContactUs, (req, res) => {
  ...
  const params = [
    ...
    language, country, company, phone, email
  ];
  ...
  pool.query(sql, params, (err, result) => {
    ...
    res.json({ success: true, message: "Form submitted successfully!", email: email });
  });
});
